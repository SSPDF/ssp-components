...
- **`index.esm.js` is ESM with a `.js` extension and no `"type"`**, so attw flags `node16 (from ESM)`. Bundlers are fine; the real fix is the `tsup` migration (Etapa 3). `main`/`exports.require` were fixed in 0.1.0.
